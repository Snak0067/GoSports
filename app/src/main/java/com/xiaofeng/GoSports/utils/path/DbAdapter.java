package com.xiaofeng.GoSports.utils.path;
...
public class DbAdapter {
    ...
    public List<PathRecord> queryRecordAll() {
        ...
        while (allRecordCursor.moveToNext()) {
            ...
            record.setPathline(RecordUtil.parseLocations(lines));
            record.setStartpoint(RecordUtil.parseLocation(allRecordCursor
                    .getString(allRecordCursor
                            .getColumnIndex(DbAdapter.KEY_STRAT))));
            record.setEndpoint(RecordUtil.parseLocation(allRecordCursor
                    .getString(allRecordCursor
    ...
    public PathRecord queryRecordById(int mRecordItemId) {
        ...
        if (cursor.moveToNext()) {
            ...
            String lines = cursor.getString(cursor
                    .getColumnIndex(DbAdapter.KEY_LINE));
            record.setPathline(RecordUtil.parseLocations(lines));
            record.setStartpoint(RecordUtil.parseLocation(cursor.getString(cursor
                    .getColumnIndex(DbAdapter.KEY_STRAT))));
            record.setEndpoint(RecordUtil.parseLocation(cursor.getString(cursor
                    .getColumnIndex(DbAdapter.KEY_END))));
        }
        return record;
    }
